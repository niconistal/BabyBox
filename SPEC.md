...
> **Current revision: v3 "Tower"** (`hardware/enclosure/babybox-case-v3.scad`
> + `keycaps.scad`) — an upright 84 x 84 x ~95 mm tower drawn from the paper
> sketches in `hardware/sketches/`. Four printable parts plus accessories:
>
...
> | **Body** | `v3 / "body"` | Open rounded tube; 4 full-height corner **columns** with a heat-set insert at each end; back power/service slot |
> | **Top plate** | `v3 / "top"` | LED **halo hub**, central figurine well, 2x **MX** button cut-outs, MFRC522 mounts (antenna-up) hanging beneath |
> | **Center lid** | `v3 / "lid"` | Magnet landing disc (pause-and-insert), engraved star; the figurine snaps here, RFID reads through it |
> | **Diffuser ring** | `v3 / "diffuser"` | Print translucent/natural — the LED strip glows through it as a halo |
...
>   the column tops, Pi/RFID screw onto insert standoffs.
> - **MX keyswitches with big round 3D-printed keycaps** instead of panel buttons.
> - **LED "halo" ring** — the SunFounder 8x WS2812B 60/m flexible strip curls
>   (centerline ~42 mm dia) around a 40 mm hub and glows through the diffuser
>   ring as a ring of light around the figurine podium.
> - **RFID + magnets move to a removable center lid** on top, reader directly
>   beneath, ~14 mm read stack (well under the ~30 mm MFRC522 range).
>
...
```
   TOP VIEW                         SIDE VIEW (the tower)
   ┌────────────────────┐           ┌────────────────────┐
   │      ___________    │          │  (●) figurine snaps │ ← center lid
   │    (  ((halo))  )   │ ← LED     │ ((  glowing halo  ))│   (magnets+RFID
   │     ( ★ lid ★  )    │   ring    ├────────────────────┤    beneath)
   │      ‾‾‾‾‾‾‾‾‾‾‾     │          │ (▶❚❚)   ( ◼ )       │ ← big round MX
   │   (▶❚❚)      ( ◼ )   │ ← round   │                    │   keycaps
   │   play       stop   │   caps    │      BODY (tube)    │
   └────────────────────┘           │     Pi + buzzer in  │
        84 x 84 mm                   │     the tray below  │
                                     │  [==] power slot ───┼─ back
   STACK (bottom → top):             └────────────────────┘
   bottom tray → body tube →                ~95 mm tall
   top plate (halo+buttons) →
   diffuser ring + center lid + figurine
```
