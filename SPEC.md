...
| WS2812B RGB 8-LED flexible strip (60/m, SunFounder kit) | Kit | -- |
| MX (Cherry-compatible) keyswitches (x2: play/pause, stop) | Purchase | ~$2 |
| Brass heat-set inserts (M3 x8, M2.5 x4, M2 x4)  | Purchase | ~$3 |
| M3 / M2.5 / M2 machine screws (assorted)        | Purchase | ~$2 |
| Passive Buzzer                   | Kit           | --        |
| Resistors / wires (soldered — no breadboard)    | Kit       | --        |
| 5V 3A micro USB power supply    | Purchase      | ~$8       |
| Mini HDMI to HDMI cable         | Purchase      | ~$5       |
| 3D-printed enclosure + figurines| Bambu Lab P2S | ~$5       |
| **Total (if speaker owned)**    |               | **~$56**  |
...
> **Wired-speaker variant:** an alternative build that replaces the Bluetooth
> speaker with a salvaged IKEA KALLSUP driver wired to a MAX98357A I2S amp —
> see [`hardware/wired-speaker/SCHEMATIC.md`](hardware/wired-speaker/SCHEMATIC.md).
...
> **Current revision: v3** (`hardware/enclosure/babybox-case-v3.scad`
> + `keycaps.scad`) — a compact 90 x 90 mm box (body only ~47 mm tall) drawn
> from the paper sketches in `hardware/sketches/`. The top is a **raised, sealed
> LED ring around a recessed hole**: the figurine drops straight in and the RFID
> reader sits flush right beneath it. Three printable parts plus accessories:
>
> | Part | File / `render_part` | Notes |
> |------|----------------------|-------|
> | **Bottom tray** | `v3 / "tray"` | Pi Zero 2 W + buzzer mount here (M2.5 / press-fit), bumper feet, floor vents, engraved logo |
> | **Body** | `v3 / "body"` | Short open tube; 4 full-height corner **columns** with a heat-set insert at each end; one tiny power-cable hole on the back, lined up with the Pi's micro-USB power port (no video/HDMI exposed) |
> | **Top plate** | `v3 / "top"` | RAISED sealed LED ring + RECESSED figurine hole; magnets in the hole floor (pause-and-insert); MFRC522 flush underneath (antenna up); 2x **MX** button cut-outs |
> | **Diffuser ring** | `v3 / "diffuser"` | Print translucent/natural; glued over the strip — caps the top + outer face so the LED strip is fully sealed |
> | **Keycaps** | `keycaps.scad / "play"`,`"stop"` | **Big round** custom caps with engraved icons, press onto the MX stems |
>
> **What changed from v2:**
> - **Soldered build, no breadboard** — everything hand-wired to the Pi header.
> - **Heat-set brass inserts everywhere** (not self-tapping) so it opens/closes
>   forever: tray bolts up into the column bottoms, top plate bolts down into
>   the column tops, Pi/RFID screw onto inserts.
> - **MX keyswitches with big round 3D-printed keycaps** instead of panel buttons.
> - **Sealed LED ring** — the SunFounder 8x WS2812B 60/m flexible strip wraps the
>   hole wall (wrap dia ~42 mm) and is fully covered by a glued translucent
>   diffuser cap: it glows as a ring, with no exposed strip for little fingers.
> - **Recessed figurine hole** — the figurine drops *into* a central hole (not
>   onto a raised lid). The MFRC522 mounts flush under the thin hole floor, so
>   the tag ends up ~5–7 mm from the antenna (well under the ~30 mm range).
>   Magnets sit in the hole floor (pause-and-insert) for a gentle snap.
> - **Only as tall as needed** — the body shrank to ~32 mm (just clears the Pi).
>
> The v2 "Pebble" (`babybox-case-v2.scad`) and the v1 design below remain for
> reference. Figurine base / magnet / RFID coin geometry is unchanged, so all
> existing figurines still work.
>
> Shared component dimensions (LED strip, MX switch/keycap, heat-set inserts)
> live in `hardware/common-params.scad`.
...
### 7.2 Enclosure Sketch (v3 "Tower")

```
   TOP VIEW                         SIDE VIEW (figurine dropped in)
   ┌────────────────────┐                     ┌─┐  ← figurine sticks up
   │     .-"""""-.      │            ___      │ │     out of the hole
   │   (  (( O ))  )    │ ← raised  /(( ))\ ──┤ ├── ← raised sealed LED ring
   │    `-.._____.-`    │   sealed ┌┴─────┴┐  └─┘     (figurine RECESSED in
   │                    │   ring   │ MFRC522│            the hole below it)
   │   (▶❚❚)     ( ◼ )   │          ├────────┤  ← RFID flush under the floor
   │   play      stop   │ ← round  │  BODY  │‹═ power hole at the Pi port
   └────────────────────┘   caps   │ tray:Pi│
        90 x 90 mm                  └────────┘
                                    body ~47 mm tall
   The figurine drops INTO the centre hole and sits recessed; the MFRC522
   reads it through the thin floor (~5 mm). The flexible LED strip wraps the
   hole wall and is fully sealed under a glued translucent diffuser cap.
```

The flexible WS2812B strip wraps the 40 mm hub; the translucent **diffuser
ring** slips over it and turns the 8 LEDs into a soft ring of light around the
figurine. The two **MX switches** carry big round printed keycaps. Everything
is held with **heat-set inserts** — four corner columns run the full height of
the body with an insert at each end (tray bolts up, top plate bolts down).
